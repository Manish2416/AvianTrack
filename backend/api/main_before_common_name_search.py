...
def multispecies_species(
    ...
    """
    Search the multi-species catalogue by scientific name.

    This catalogue represents the controlled AvianTrack sample,
    not the complete GBIF occurrence population.
    """
    ...
    for _, row in df.iterrows():
        ...
        records.append({
            ...
            "first_observation": (
                ...
                else first_date.strftime("%Y-%m-%d")
            ),

            "last_observation": (
                None
                if pd.isna(last_date)
                else last_date.strftime("%Y-%m-%d")
            ),
            ...
            "iucn_category": (
                None
                if pd.isna(row["iucn_category"])
                else str(row["iucn_category"])
            )
        })
